...
must_haves:
  ...
  artifacts:
    ...
    - path: "tests/conftest.py"
      provides: "新增 mock_llm + test_client fixtures"
  key_links:
...
<objective>
编写完整单元测试套件（FastAPI 路由、LLM Adapter、ChatService、TaskService）、创建统一 .bat 启动脚本、更新 .gitignore 排除构建产物。

Purpose: 完成 Phase 7 的全部测试基础设施和运维脚本，使系统可从 .bat 脚本一键启动。

Output: 单元测试全部通过、start-web.bat 脚本可用、最终项目配置就绪。
</objective>
...
<task type="auto" tdd="true">
  ...
  <read_first>
    - tests/conftest.py — [why: 现有测试 fixtures 结构，需要新增 mock_llm 和 test_client]
    ...
    - .planning/phases/07-web-ui-ai-integration-mcp-e2e-testing/07-VALIDATION.md — [why: Wave 0 测试缺口和测试命令]
  </read_first>
  <behavior>
  ...
  <action>
    创建 4 个测试文件和 1 个 conftest 更新。

    1. **`tests/conftest.py`** — 追加新 fixtures（不修改现有 fixtures）。
       在文件末尾追加：
       ```python
       @pytest.fixture
       def mock_llm():
       ...
       @pytest.fixture
       def test_client():
           """FastAPI TestClient with mock LLM provider."""
       ...
       def task_store(tmp_path):
           """TaskStore with temporary SQLite DB."""
    ...
    2. **`tests/unit/api/test_routes.py`** — FastAPI 路由单元测试。
       ```python
    ...
    3. **`tests/unit/adapters/test_llm_adapter.py`** — ILLMProvider 单元测试。
       ```python
    ...
    4. **`tests/unit/services/test_chat_service.py`** — ChatService 单元测试。
       ```python
    ...
    5. **`tests/unit/services/test_task_service.py`** — TaskStore SQLite 单元测试。
       ```python
