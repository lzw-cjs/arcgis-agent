...
# src/arcgis_agent/api/routes/tools.py → 31 个工具端点
    ...
    2. **`tests/unit/adapters/test_llm_adapter.py`** — ILLMProvider 单元测试。
       ...
       class TestGISTools:
           ...
           def test_tool_count_matches(self):
               """Verify tool count is >= 31 (D-04: full coverage)."""
               assert len(ALL_GIS_TOOLS) >= 31, \
                   f"Expected >= 31 tools, got {len(ALL_GIS_TOOLS)}"
       ```
...
- `tests/unit/api/test_routes.py` 覆盖所有 31+ 工具端点和 chat/tasks/upload/health 端点
