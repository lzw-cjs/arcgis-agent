...
- **工程化要求**：
  - 完整的类型注解（typing），所有公开 API 必须有类型签名
  - 错误处理分级：预期错误（用户输入错误、文件不存在）返回结构化错误；意外错误（arcpy 异常）记录日志并优雅降级
  - 日志系统：统一 logging 配置，支持 --verbose / --quiet 控制级别
  - 配置管理：支持 config 文件（项目级 > 用户级 > 全局默认）
  - 输入验证：所有用户输入在调用 arcpy 前必须验证（路径存在性、参数范围、格式合法性）
  - 单元测试覆盖核心逻辑，集成测试覆盖 arcpy 调用链
  - CI/CD 就绪：可被自动化流水线调用，退出码语义明确
...
- **工程化稳定性**：
  ...
  - 错误不能静默吞掉，必须结构化返回或记录日志
  - 每个命令必须有 --help 说明和参数验证
  - 退出码必须语义明确（0=成功, 1=用户错误, 2=系统错误, 3=arcpy 错误）
  - 不允许裸 except，必须捕获具体异常类型
  - 文件路径处理必须使用 pathlib，避免字符串拼接的跨平台问题
